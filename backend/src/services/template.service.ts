...
export async function create(data: Partial<Template>): Promise<Template> {
  ...
  return { ...doc, _id: result.insertedId } as unknown as Template;
}
...
export async function findAll(params: FindAllParams) {
  ...
  const templates = await db.collection(templateCollection)
    .find(query)
...
export async function findById(id: string): Promise<Template | null> {
  const db = getDb();
  return db.collection(templateCollection).findOne({ _id: new ObjectId(id) }) as Promise<Template | null>;
}
